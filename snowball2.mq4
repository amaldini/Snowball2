   ...
   if (isLong||isShort) { 
      ...
      if (RENKO_AUTO_TRADE && !bigSpread) {
         ...
         if (profit>0 && profit>2) { // <--- chiude solo con un profitto significativo, 
                                     ...
                                     //      con guadagno nullo e prezzi sempre pi� sfavorevoli
            if (isLong) {
               /*
         ...
         if (needToClose) {
            ...
            */
            ...
            if (RENKO_PYRAMID_Pips>0) {
               ...
               // che � dato dal primo_buy_price+RENKO_PYRAMID_PIPS*(nOpenTrades),
               // apro un nuovo trade
...
void checkBreakEven2() {
   ...
   // TODO: armed � da calcolare al volo per ogni trade (non mettere in variabile statica)
   ...
   while (doCycle) {
      ...
      for (int cnt = 0; cnt < total; cnt++) {      
         ...
         if (isMyOrder(magic)) {
               ...
               double orderPrice; // lo calcolo in base allo stoploss perch� dopo un resume 
      ...
      for (cnt = 0; cnt < total; cnt++) {      
         ...
         if (isMyOrder(magic)) {
               ...
               if (isToClose) {
                  ...
                  orderCloseReliable(OrderTicket(), OrderLots(), 0, 999, clr);
                  maxLevel = 0; // <== verr� ricalcolato successivamente
                  doCycle=true;
                  break; // cycle again starting from 0 (HELLO FIFO!)
               }
            }
...
void checkForStopReduction() {   
   ...
   if (level==0) { // SOLO SE LEVEL=0 VALUTIAMO SE CAMBIARE LO STOP!!!!
      ...
      // altrimenti riduco di 1 pip all'ora (solo se il prezzo � sufficientemente lontano
