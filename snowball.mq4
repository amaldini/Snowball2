...
void onTick(){
   recordEquity(name+Symbol6(), PERIOD_H1, -1);
...
void resume(){
   ...
   if (level > 0){
      resumeInfo = "RESUMING LONG from level:"+level;
      for (i=1; i<=level; i++){
         sl = line - pip * i * stop_distance;
         resumeInfo=resumeInfo+"\n"+" buying at "+NormalizeDouble(Ask,5)+" with stop loss="+sl;
         buy(lots, sl, 0, magic, comment);
      }
   }
   
   if (level < 0){
      resumeInfo = "RESUMING SHORT from level:"+(-level);
      for (i=1; i<=-level; i++){
         sl = line + pip * i * stop_distance;
         resumeInfo=resumeInfo+"\n"+" selling at "+NormalizeDouble(Bid,5)+" with stop loss="+sl;
   ...
   if (level == 0 && start_immediately) {
      if (direction == SHORT) {
         placeLine(Bid + pip * stop_distance);
      }
      if (direction == LONG) {
         placeLine(Ask - pip * stop_distance);
      }
   } 
...
void checkButtons(){
   if(!running){
      ...
      if (labelButton("long_now", 15, 60, 1, "long now", Green)){
         start_immediately = true;
         go(LONG);
      }
      
      if (labelButton("short_now", 15, 75, 1, "short now", Red)) {
         start_immediately = true;
         go(SHORT);
      }
